...
How to convert a `deck.md` slide into a `gpt-image-2` request using Codex OAuth/Codex image tooling first. This file defines the prompt structure, token injection, and guardrails that produce consistent designer-mode slides. Direct OpenAI Image API-key calls are a fallback only after the Codex-authenticated path is unavailable or fails and the human explicitly approves continuing with the API-key fallback.
...
## Generation auth path and fallback

Default path:
- Use `gpt-image-2` through Codex OAuth/Codex image tooling for designer-mode slide images.
...
Fallback path:
- Do not start with a direct OpenAI API key.
- If Codex OAuth/Codex image tooling is unavailable, errors, or cannot accept the required prepared inputs, tell the human what failed.
- Ask the human whether to continue with the direct OpenAI Image API key fallback.
- Only after approval, use `image_generation.fallback_auth` and record the fallback approval and reason in `method/model-inputs.yaml` or the slide request metadata.
