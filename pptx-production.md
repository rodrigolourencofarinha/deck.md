...
2. write or refine the approved Unified Specsheet
3. for template-driven slides, read `references/template-catalog.md` and suggest the best template for each slide
4. for coded slides, run `scripts/build_pptx.py OUTPUT_PPTX SPEC_YAML [TEMPLATE_PPTX]`
...
The Unified Specsheet stays the only user-facing planning artifact.
Do not invent a second planning schema.
...
Keep the template suggestion inside the Unified Specsheet so the user can approve the choice with the rest of the slide logic.
...
3. Add the selected template file and rationale to the Unified Specsheet.
...
2. run `scripts/build_pptx.py OUTPUT_PPTX SPEC_YAML [TEMPLATE_PPTX]`
