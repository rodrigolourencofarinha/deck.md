...
**This document (`SPEC.md`) is the authoritative specification of the format.** For a quick-start brief to copy and fill in, see [`./deck.md`](./deck.md) or [`./deck.minimal.md`](./deck.minimal.md). For the richer starter, see [`./deck.full.md`](./deck.full.md). For worked examples, see [`./examples/`](./examples/).
...
|---|---|---|---|
| **Minimal** | 3–10 slide SCR, update, pitch | Narrative template + slide titles | [`deck.md`](./deck.md), [`deck.minimal.md`](./deck.minimal.md), [`examples/scr.deck.md`](./examples/scr.deck.md) |
| **Standard** | Most working decks | Slide bodies, sources, speaker notes | [`examples/update.deck.md`](./examples/update.deck.md) |
| **Full** | Client-facing, image-led | `designer_assets`, `creative_direction`, `required_text`, `chart`, full `key_line` | [`deck.full.md`](./deck.full.md), [`examples/pyramid.deck.md`](./examples/pyramid.deck.md) |
...
- `designer-mode` slides are generated with `gpt-image-2` through the Codex OAuth/Codex image path first, using the prompt templates in [`./standards/image-prompts.md`](./standards/image-prompts.md). The direct OpenAI Image API key path is a fallback only after Codex OAuth or the Codex image path fails and the human explicitly approves continuing with the API-key fallback.
