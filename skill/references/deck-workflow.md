...
For designer-mode images, use `gpt-image-2` through Codex OAuth/Codex image tooling first. If that auth/transport path fails or cannot accept the required inputs, tell the human what failed and ask whether to continue with the direct OpenAI Image API key fallback.
